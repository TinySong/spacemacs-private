...
(defun easy-hugo ()
  ...
  (easy-hugo-with-env
   ...
   (unless easy-hugo--refresh
     (setq easy-hugo--cursor (point)))
   (let ((files (find-lisp-find-files (expand-file-name easy-hugo-postdir easy-hugo-basedir) ".org"))
         (lists (list)))
       ...
       (progn
         ...
         (while files
           (unless (or (string= (car files) ".")
                       (string= (car files) ".."))
             ;; (if (file-directory-p (concat easy-hugo-basedir easy-hugo-postdir (car files)))
             ;;     (progn
             ;;       (setq file-subdir (directory-files (expand-file-name (car files) (concat easy-hugo-basedir easy-hugo-postdir))))
             ;;       (message "--------------")
             ;;       (message "%s" file-subdir)
             ;;       (push file-subdir files)))
             (when (member (file-name-extension (car files)) easy-hugo--formats)
               (push
                (concat
                 (format-time-string "%Y-%m-%d %H:%M:%S "
                                     (nth 5 (file-attributes
